...
[reference](https://www.cnblogs.com/ainyi/p/9777841.html)
...
- 主垃圾回收器负责老生代的垃圾回收，老生代对象占用空间大，对象存活时间长。
- 副垃圾回收器负责新生代的垃圾回收。
...
5. 缓存对象未清理（使用 Map、Set 不当）

### 优化内存问题的方式
- chrome 提供了 Performance → Memory 面板；
- 使用Heap Snapshot（堆快照） 分析对象数量；
- 避免无用闭包；
- 及时清除事件监听；
- 使用 WeakMap / WeakSet；
