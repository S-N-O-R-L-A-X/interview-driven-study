...
- 最早的 DOM0 级模型，这种模型不会传播，所以没有事件流的概念，但是现在有的浏览器支持以冒泡的方式实现，它可以在网页中直接定义监听函数，也可以通过 js 属性来指定监听函数。这种方式是所有浏览器都兼容的。
...
- DOM2 级事件模型，在该事件模型中，一次事件共有三个过程，第一个过程是事件捕获阶段。捕获指的是事件从 document 一直向下传播到目标元素，依次检查经过的节点是否绑定了事件监听函数，如果有则执行。后面两个阶段和 IE 事件模型的两个阶段相同。这种事件模型，事件绑定的函数是 `addEventListener`，其中第三个参数可以指定事件是否在捕获阶段执行。默认是false，在冒泡阶段执行。
...
事件委托是利用浏览器事件冒泡的机制来优化事件处理的技术。因为事件在冒泡过程中会上传到父节点，并且父节点可以通过事件对象获取到目标节点，因此可以把子节点的监听函数定义在父节点上，由父节点的监听函数统一处理多个子元素的事件。
...
### 原型
在 JavaScript 里，除了 `null` 之外，每个对象都有一个内部隐藏的指针，指向另一个对象，这个被指向的对象就叫“原型”。

规范中用 [[Prototype]] 表示这个指针，而在代码中我们通过 `__proto__`（浏览器环境）或 `Object.getPrototypeOf()` 获取对象原型。

### 原型链
当访问一个对象的属性时，如果这个对象内部不存在这个属性，那么它就会去它的原型对象里找这个属性，一直找到顶层的 `Object.prototype`，最后到 `null` 为止。这一条链接路径，就是原型链。

这就是新建对象能够使用`toString()`等方法的原因。

### `prototype`

只有函数才有 `prototype` 属性。
每个函数在创建时，都会自动获得一个 `prototype` 对象，里面只有一个属性 `constructor`，指回函数自身。
当用 `new` 调用这个函数（当作构造函数）时，生成的新对象的 [[Prototype]]（即 `__proto__`）就会指向这个函数的 `prototype` 对象。

### `__proto__`

任何一个对象（包括普通对象、数组、函数等）都有 `__proto__`，它指向该对象的构造函数的 `prototype`，包含了可以由该构造函数的所有实例共享的属性和方法。
...
## V8引擎
`V8` 是由 Google 开发的开源JavaScript和 webassembly 引擎，是一种 JavaScript 的虚拟机。

### 组成
内部主要有四大模块
- `Parse` 将 JavaScript 源码转换为 Abstract Syntax Tree (AST)
- `Ignition` 将 AST 转换为 Bytecode 并解释执行
- `TurboFan(compiler)` 将 Bytecode 转换为优化的汇编代码
- `Orinoco(GC)` 负责垃圾回收

### 运行方式
V8 执行 JavaScript 代码所经历的主要流程包括：
...
### 解释执行 vs 编译执行
- `解释执行`将输入的源代码通过解析器编译成中间代码，之后直接使用解释器解释执行中间代码，然后直接输出结果。
- `编译执行`将源代码转换为中间代码，然后编译器再将中间代码编译成机器代码。通常编译成的机器代码是以二进制文件形式存储，需要执行这段程序的时候直接执行二进制文件。

`解释执行`启动速度快，但是执行时速度慢，而`编译执行`启动速度慢，但是执行速度快。
在 V8 出现之前，所有的 JavaScript 虚拟机都采用解释执行。而 V8 率先引入即时编译（JIT）的设计，采用权衡策略，在启动过程中采用解释执行，当某段代码的执行频率超过额定值时优化编译器将其编译成执行效率更加高效的机器代码。

reference: https://www.zhihu.com/tardis/zm/art/441313455?source_id=1005