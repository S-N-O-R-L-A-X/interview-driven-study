...
* `z-index` 只在属性 `position: relative/absolute/fixed` 的时候才生效。
* `z-index` 默认值是auto
...
- 重排（回流）reflow 当页面的元素的位置、尺寸等发生改变时，浏览器会重新计算该元素的几何属性并摆放到正确的位置的过程叫做重排。
- 重绘 repaint 当节点不影响布局的属性发生变化，只需要重新计算节点在屏幕中的绝对位置并渲染的过程，就叫重绘。
重排的过程在重绘的过程前面，所以重排一定会重绘，但重绘不一定会引起重排。

为了提升性能，浏览器通常都会将多次回流操作放入一个队列中，等过了一段时间或操作达到了一定的临界值才会挨个执行。但是，当访问布局信息的时候，会出现强制回流。
...
- 修改元素属性时一次性进行修改，通过`elem.style.cssText`一次性修改；
- 让dom脱离标准流（比如设置`display:none`）后进行操作，然后再放回标准流；
- 避免访问一些引起回流的属性或方法
