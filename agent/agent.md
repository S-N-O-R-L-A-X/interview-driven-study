- [Agent](#agent)
  ...
  - [memory](#memory)
    - [使用方式](#使用方式)
  - [agent 开发状态幂等、防止任务重复执行的机制](#agent-开发状态幂等防止任务重复执行的机制)
...
## agent 开发状态幂等、防止任务重复执行的机制

1. 每个任务或操作被创建时生成唯一标识符key。
2. 执行前，在数据库中查询该 key 是否存在已完成记录。
3. 若不存在，插入一条状态为“处理中”的记录（利用唯一索引防并发冲突），然后执行业务逻辑。
4. 业务成功后将记录状态改为“已完成”，并保存执行结果；下次相同 key 直接返回已有结果，不执行业务。