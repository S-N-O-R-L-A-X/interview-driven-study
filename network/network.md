- [Network](#network)
	...
	- [MCP 使用协议前后对比](#mcp-使用协议前后对比)
		- [为什么不用websocket？](#为什么不用websocket)
	- [HTTP 2.0 vs HTTP 3.0](#http-20-vs-http-30)
		- [更换原因](#更换原因)
		- [队头阻塞问题](#队头阻塞问题)
	- [ETag](#etag)
		- [使用方式](#使用方式)
		- [强验证 ETag（Strong Validator）](#强验证-etagstrong-validator)
		- [弱验证 ETag（Weak Validator）](#弱验证-etagweak-validator)
		- [`ETag` vs `Last-Modified`](#etag-vs-last-modified)

# Network

## HTTP和HTTPS的异同/HTTPS的缺点
- HTTP是超文本传输协议，其中的信息是明文传输的。HTTPS是HTTP协议再加上SSL（安全套接字层），具有安全性。
- HTTP不验证通信方的身份，通信方的身份有可能遭遇伪装；HTTPS需要到CA（证书颁发机构）申请证书，一般免费证书较少，因而需要一定费用。
- HTTP和HTTPS使用的是完全不同的连接方式，用的端口也不一样，前者是80，后者是443。
- HTTP无法证明报文的完整性，报文有可能遭篡改。
- HTTP的连接很简单，是无状态的；HTTPS协议是由SSL+HTTP协议构建的可进行加密传输、身份认证的网络协议，比HTTP协议安全。
- HTTPS连接缓存不如HTTP高效，流量成本高。
- HTTPS连接服务器端资源占用高很多，支持访客多的网站需要投入更大的成本。
- HTTPS协议在传输数据时需要进行加密和解密的操作，延长页面的加载时间，增加耗电。
...
<table>
	...
	<tr>
		<td>代理对象</td>
		<td>代理客户端, 服务端不知道实际发起请求的客户端</td>
		<td>代理服务端, 客户端不知道实际提供服务的服务端</td>
	</tr>
	<tr>
		<td>作用</td>
		<td>
...
4. 代理可以对外隐藏用户信息
		...
		<td>
1. 保证内网的安全，阻止web攻击，大型网站，通常将反向代理作为公网访问地址，Web服务器是内网。<br/>
...
#### node 正向代理
在本地开发环境中，由配置的开发服务器（如Webpack Dev Server）充当正向代理。所有API请求都先发往这个同源的开发服务器，再由它转发给真实的后端API。
主流框架都可以通过配置proxy来解决，虽然是不同框架，但基本都用的http-proxy-middleware。

#### nginx 反向代理
反向代理服务器（如Nginx）和你的网页同源。浏览器请求代理服务器，它去请求真实的目标服务器。在浏览器看来，它一直在和同一个“源”通信，所以不触发跨域限制。
...
- 带 cookie 请求：前后端都需要设置。前端设置`withCredentials`为 `true`,后端设置`Access-Control-Allow-Credentials`为 `true`, 同时`Access-Control-Allow-Origin`不能设置为`*`

#### websocket

普通AJAX请求时，浏览器会触发 CORS 预检，先发送一个 OPTIONS 预检请求，询问服务器是否允许跨域。只有服务器返回了正确的 CORS 头，浏览器才会发送真正的请求。
而 WebSocket 设计时没有引入同源策略限制。它通过 HTTP Upgrade 机制完成握手请求，不触发 CORS 预检，浏览器会直接向任意域名的 WebSocket 服务器发起连接请求。这个握手请求中会携带 Origin 头，标明请求来自哪个源（比如 http://www.a.com）。服务器可以检查这个 Origin 值，决定是否接受连接。
所以，WebSocket 的跨域安全完全交给服务器控制。
...
### 为什么不用websocket？
- MCP (基于Streamable HTTP)追求"无状态"。每个请求是独立的，服务器处理完即忘，天然适合Serverless和云环境。WebSocket 本质上"有状态"。客户端与服务器间需维持一个长期连接。
- websocket断开时会话会丢失，需要手动实现"断线重连"和"状态恢复"逻辑。Streamable HTTP可以通过会话ID（Mcp-Session-Id）恢复会话。
- 每个 WebSocket 连接在服务端都需要独立的内存结构来存储连接上下文、缓冲区、定时器等，且每个连接至少占用一个 socket 文件描述符。高并发情况下资源消耗巨大。

当然，官方协议允许使用自定义传输，所以websocket 也可以作为一个选择。
...
## ETag
ETag（Entity Tag，实体标签）是 HTTP 协议的一部分，它是服务器为特定版本的资源生成的一个唯一标识字符串。
当资源内容发生变化时，服务器会生成一个全新的 ETag 值。反之，如果资源内容没有变化，ETag 值就会保持不变。
主要用途：用于协商缓存，以高效地判断客户端缓存的文件是否与服务器上的文件一致。

### 使用方式
服务器返回资源，并在响应头中加上 ETag 字段。
浏览器会缓存这个资源和它的 ETag 值。
当强缓存过期，浏览器需要再次请求该资源时，它会在请求头中带上之前收到的 `ETag` 值，这个头叫做 `If-None-Match`。

### 强验证 ETag（Strong Validator）
要求字节完全相等。只要资源有一个字节不同，ETag 值就必须改变。
通常用双引号直接表示，例如：ETag: "123456"
适用于需要精确匹配的场景，例如文件下载。

### 弱验证 ETag（Weak Validator）
在值前加上 W/ 前缀，表示“弱”验证。
它只要求资源的语义内容相等，而允许一些非实质性的变化（例如，页脚的广告内容变了，或者注释有改动）。
例如：ETag: W/"123456"
能避免因不重要的更改而导致缓存失效。

### `ETag` vs `Last-Modified`
* ETag 基于文件内容生成，与时间无关。
* 服务器可以完全控制 ETag 的生成算法，因此有时甚至可以忽略一些无关紧要的改动（使用弱 ETag）。

而`Last-Modified`有四个问题
1. 只能精确到秒。如果文件在1秒内被多次修改，它将无法准确标识最新版本。
2. 可能导致不必要的重新下载，比如文件重新部署后修改时间更新。
3. 服务器时钟不同步时可能会出问题.
4. 无法感知实质性变化：无法区分页面的一个广告位变化和文章主体内容变化。